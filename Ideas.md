...
### Betroffene Module
- `statistik.js` – neue Sektion unterhalb des Charts
...
des bestehenden Charts ein. Keine anderen Sektionen verändern.
...
## Idee 3 – Ausschlussdiät-Zeitplan / Phasentracker
...
### Betroffene Module
- `tagebuch.js` – erweiterter Ausschluss-Submit um `phase` und `phase_start`
- `ansicht.js` – Phasen-Banner oberhalb der Ausschluss-Einträge
- `statistik.js` – Phasen-Timeline in der Ausschlussdiät-Sektion
- `index.html` – neue Felder im Ausschluss-Formular
...
4. Phasen-Timeline in statistik.js (Ausschlussdiät-Sektion):
...
## Idee 4 – Tierarzt-Export als PDF

### Beschreibung
Kompakten, druckbaren Bericht als PDF direkt im Browser generieren – ohne Backend, ohne externe API. Enthält: Deckblatt mit Hund-Stammdaten, Symptomverlauf der letzten 90 Tage als Chart-Screenshot, aktuelle Medikamente, bekannte Allergene, Ausschlussdiät-Status. Alle Daten liegen bereits vor.

### Betroffene Module
- Neues Modul `export.js` – PDF-Generierung via `window.print()` + CSS `@media print`
...
Ich teile jetzt statistik.js, store.js, cache.js und styles.css.
...
1. Neues Modul js/export.js mit Funktion exportTierarztPDF(hundId, zeitraumTage=90):
   ...
   - Ruft Daten aus store.js (Stammdaten, Medikamente, Allergene, Ausschluss) und
     cache.js (Symptomtagebuch, Futtertagebuch) ab
   - Inhalt:
...
## Idee 5 – Rezept-Nährstoffvergleich (A vs. B)
...
### Betroffene Module
- `rechner.js` – neue Funktion `showVergleich()` + Vergleichs-Panel
- `index.html` – Vergleichs-Panel HTML
...
## Idee 6 – Symptom-Muster-Erkennung (Wochentag / Monat)
...
### Betroffene Module
- `statistik.js` – neue Sektion „📅 Symptom-Muster" unterhalb der Korrelationsanalyse
...
unterhalb der bestehenden Korrelationsanalyse (falls vorhanden) bzw. unterhalb der Chart-Sektion.
...
## Idee 8 – Zutat-Datenbank-Import (USDA / Open Food Facts)
...
## Idee 9 – Mehrere Hunde – getrennte Statistik-Dashboards
...
## Idee 10 – Reaktions-Score pro Zutat
...
am Ende der Statistik-Seite ein. Keine anderen Sektionen verändern.
...
Wenn du mehrere Features implementieren möchtest, empfehle ich diese Reihenfolge nach steigender Komplexität und Abhängigkeiten:

| Priorität | Feature | Aufwand | Abhängigkeiten |
|-----------|---------|---------|----------------|
| 1 | Idee 6 – Symptom-Muster | Gering | Nur statistik.js |
| 2 | Idee 2 – Korrelationsanalyse | Gering | Nur statistik.js |
| 3 | Idee 10 – Reaktions-Score | Mittel | statistik.js + store.js |
| 4 | Idee 5 – Rezept-Vergleich | Mittel | rechner.js + index.html |
| 5 | Idee 9 – Mehrere Hunde | Mittel | statistik.js + index.html |
| 6 | Idee 4 – Tierarzt-PDF | Mittel | Neues Modul export.js |
| 7 | Idee 8 – USDA-Import | Hoch | stammdaten.js + config.js + ext. API |
| 8 | Idee 3 – Phasentracker | Hoch | Neues Sheet + 3 Module |
